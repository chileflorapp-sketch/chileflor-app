...
export default function CreaDedicatoria({ params }: { params: { orderId: string } }) {
  ...
  const handleSave = async () => {
    ...
    if (cardEl) {
      try {
        ...
        // Upload to Cloud for Temporary Page
        const res = await fetch('/api/tarjeta/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ image: dataUrl, orderId: params.orderId })
        });
        ...
        if (res.ok) {
          ...
          console.error('Failed to upload image to cloud');
        }

      } catch (err) {
        console.error('Error capturando tarjeta:', err);
      }
    }
  ...
  if (qrGenerated) {
    const cardBg = GALLERY_IMAGES[selectedCard];
    
    const shareToWhatsApp = () => {
      if (!tarjetaId) {
        alert("La tarjeta aún se está procesando. Por favor, intenta en unos segundos.");
        return;
      }
      
      try {
        const linkMagico = `${window.location.origin}/t/${tarjetaId}`;
        const text = `¡Hola! Alguien muy especial te preparó una sorpresa increíble. Ábrela aquí antes de que desaparezca: ${linkMagico}`;
        
        // Intentar usar Web Share API nativo si está disponible
        if (navigator.share) {
          navigator.share({
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 border border-gray-100 shadow-2xl flex flex-col items-center text-center">
          ...
          <p className="text-gray-500 mb-6">Aquí está el resultado final para el pedido #{params.orderId}.</p>
          
          {cardImage && (
            <div className="mb-8 w-48 rounded-xl overflow-hidden shadow-2xl ring-4 ring-gray-100 transform -rotate-2">
          ...
          <button 
            ...
            className="w-full mt-4 bg-transparent text-gray-500 font-bold py-4 rounded-xl hover:bg-gray-50 transition-all active:scale-95"
          >
            Volver a editar
          </button>
        </div>
  ...
  return (
    <div className="bg-[#F9FAFB] flex flex-col min-h-screen">
      ...
      <div className="w-full md:w-1/2 lg:w-5/12 bg-gray-100 md:bg-gray-200/50 relative border-l border-gray-200 p-4 md:p-8 flex flex-col justify-between h-auto md:h-screen sticky top-0">
        ...
        <div className="flex-grow flex items-center justify-center relative w-full h-[50vh] md:h-auto">
          ...
          <div id="tarjeta-preview" className="w-[280px] sm:w-[320px] aspect-[3/4] rounded-2xl flex flex-col justify-between p-6 relative overflow-hidden transition-all duration-700 shadow-2xl group border border-white/20 bg-white">
            ...
            <div className="flex flex-col items-center gap-2 mt-auto z-10">
              <div className="w-20 h-20 bg-white rounded-xl p-1.5 shadow-xl ring-2 ring-white/50">
                 <div className="w-full h-full bg-[url('https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=http://localhost:3000/muro/mock-hash-123')] bg-contain bg-no-repeat bg-center opacity-90"></div>
              </div>
        ...
        {/* Action Button - Sticky at bottom for mobile, absolute for desktop */}
        <div className="mt-6 md:mt-0 relative z-20">
          ...
          >
            <span>{isCapturing ? 'Procesando...' : 'Generar Experiencia'}</span>
            {!isCapturing && <svg className="w-6 h-6 transform group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>}
          </button>
        </div>
